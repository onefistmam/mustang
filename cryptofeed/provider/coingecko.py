...
class Coingecko(RestFeed):
    ...
    async def _market_info(self, session, pair):
        ...
        async with session.get("{self.address}coins/{quote_c}?localization=false&tickers=false&market_data=true&community_data=true&developer_data=false&sparkline=false") as response:
