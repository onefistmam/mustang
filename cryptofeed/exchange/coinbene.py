...
class Coinbene(RestFeed):
    ...
    async def _trades(self, session, pair):
        ...
        if pair not in self.last_trade_update:
            async with session.get("{self.address}trades?symbol={pair}") as response:
                data = await response.json()
                self.last_trade_update[pair] = timestamp_normalize(self.id, data['trades'][-1]['time'])
        else:
            async with session.get("{self.address}trades?symbol={pair}&size=2000") as response:
                data = await response.json()
    ...
    async def _ticker(self, session, pair):
        ...
        """
        async with session.get("{self.address}ticker?symbol={pair}") as response:
            data = await response.json()
