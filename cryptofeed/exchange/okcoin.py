...
class OKCoin(Feed):
    ...
    def __calc_checksum(self, pair):
        ...
        bids = ("{bid}:{self.l2_book[pair][BID][bid]}" for bid in bid_it)
        bids = list(islice(bids, 25))
        asks = ("{ask}:{self.l2_book[pair][ASK][ask]}" for ask in ask_it)
        ...
        if self.config:
            for chan in self.config:
                if chan == LIQUIDATIONS:
                    continue
                args = ["{chan_format(chan, pair)}:{pair}" for pair in self.config[chan]]
                await websocket.send(json.dumps({
                    "op": "subscribe",
                    "args": args
                }))
        else:
            chans = ["{chan_format(chan, pair)}:{pair}" for chan in self.channels for pair in self.pairs if chan != LIQUIDATIONS]
            await websocket.send(json.dumps({
                "op": "subscribe",
                "args": chans
            }))
