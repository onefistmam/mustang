...
class OKEx(OKCoin):
    ...
    def get_active_option_contracts_info(underlying: str):
        return requests.get(OKEx.api + 'option/v3/instruments/{underlying}').json()
        ...
        async with aiohttp.ClientSession() as session:
            while True:
                for pair in pairs:
                    ...
                    for status in (0, 1):
                        end_point = "{self.api}{instrument_type}/v3/instruments/{pair}/liquidation?status={status}&limit=100"
