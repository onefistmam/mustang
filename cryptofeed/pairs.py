...
def bybit_pairs():
    ...
    for pair in r['result']:
        normalized = "{pair['base_currency']}{PAIR_SEP}{pair['quote_currency']}"
...
def kraken_pairs():
    ...
    for pair in data['result']:
        ...
        normalized = "{base}{PAIR_SEP}{quote}"
...
def huobi_dm_pairs():
    ...
    pairs = {}
    for e in r['data']:
        pairs["{e['symbol']}_{mapping[e['contract_type']]}"] = e['contract_code']
        _exchange_info[HUOBI_DM]['tick_size'][e['contract_code']] = e['price_tick']
        _exchange_info[HUOBI_DM]['short_code_mappings']["{e['symbol']}_{mapping[e['contract_type']]}"] = e['contract_code']
...
def coinbene_pairs():
    ...
    return {"{e['baseAsset']}{PAIR_SEP}{e['quoteAsset']}": e['ticker'] for e in r['symbol']}


def bittrex_pairs():
    r = requests.get('https://api.bittrex.com/api/v1.1/public/getmarkets').json()
    r = r['result']
    return {"{e['MarketCurrency']}{PAIR_SEP}{e['BaseCurrency']}": e['MarketName'] for e in r if e['IsActive']}


def bitcoincom_pairs():
    r = requests.get('https://api.exchange.bitcoin.com/api/2/public/symbol').json()
    return {"{data['baseCurrency']}{PAIR_SEP}{data['quoteCurrency'].replace('USD', 'USDT')}": data['id'] for data in r}


def bitmax_pairs():
    r = requests.get('https://bitmax.io/api/v1/products').json()
    return {"{data['baseAsset']}{PAIR_SEP}{data['quoteAsset']}": data['symbol'] for data in r}


def upbit_pairs():
    r = requests.get('https://api.upbit.com/v1/market/all').json()
    return {"{data['market'].split('-')[1]}{PAIR_SEP}{data['market'].split('-')[0]}": data['market'] for data in r}
...
def deribit_pairs():
    ...
    for c in currencies:
        for k in kind:
            data.extend(requests.get("https://www.deribit.com/api/v2/public/get_instruments?currency={c}&expired=false&kind={k}").json()['result'])
...
def coingecko_pairs():
    ...
    return {("{q['symbol']}{PAIR_SEP}{bk}".upper() if q['symbol'] not in normalized else "{normalized[q['symbol']]}{PAIR_SEP}{bk}".upper()): "{q['id']}_{bv}" for q in quote_c for bk, bv in base_c}
