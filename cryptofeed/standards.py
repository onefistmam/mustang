...
def pair_std_to_exchange(pair: str, exchange: str):
    ...
    LOG.info("standards pair_std_to_exchange, pair:%s, exchange:%s, _std_trading_pairs:%s", pair, exchange, _std_trading_pairs)
    if exchange in {BITMEX, DERIBIT, KRAKEN_FUTURES}:
        return pair

    from befh.core.PairConverter import PairConverter
    pair = PairConverter.convertPair(pair=pair, exchange=exchange)
    LOG.info("pair_std_to_exchange pair=%s, exchange=%s", pair, exchange)

    if pair in _std_trading_pairs:
        try:
            return _std_trading_pairs[pair][exchange]
        except KeyError:
            raise UnsupportedTradingPair('{pair} is not supported on {exchange}')
    else:
...
def feed_to_exchange(exchange, feed, silent=False):
    def raise_error():
        exception = UnsupportedDataFeed("{feed} is not currently supported on {exchange}")
        if not silent:
